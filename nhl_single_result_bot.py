...
# sports.ru slugs (для опциональной кириллицы)
SPORTSRU_SLUG = {
...
def fetch_scoring_official(gamePk:int, home_tri:str, away_tri:str)->List[ScoringEvent]:
    ...
    for i in idxs:
        ...
        p=allPlays[i] or {}
        res=p.get("result",{}) or {}
        if _upper_str(res.get("eventTypeId"))!="GOAL":
            continue
        ...
        # players array — надёжный источник
        scorer=""
        assists:List[str]=[]
        ...
        # fallbacks (редко нужны)
        if not scorer:
            det_candidates = (
                "scoringPlayer","scorerName","scoringPlayerName","shootingPlayerName",
        ...
        if not scorer:
            # любое *name без goalie/team/coach
            for source in (res, p):
    ...
    for key in sorted(groups.keys(), key=sort_key):
        ...
        if t=="REGULAR":
            ...
            if not per:
                ...
                # победный буллит — последний забитый в SHOOTOUT
                last = per[-1]
                who = last.scorer or "—"
